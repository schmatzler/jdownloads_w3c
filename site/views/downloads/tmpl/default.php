    ...
    if ($layout_files_text != ''){
        ...
        if (isset($user_can_see_download_url)){ 
            $checkbox_top = '<tr><form name="down'.$formid.'" action="'.JRoute::_('index.php?option=com_jdownloads&amp;view=summary&amp;Itemid='.$file_itemid).'"
                    ...
                    <td width="89%" align="right">'.JDHelper::getOnlyLanguageSubstring($jlistConfig['checkbox.top.text']).'</td>
            ...
            if ($total_downloads > 0){
                $body = str_replace('{checkbox_top}', '<div style="text-align:center; padding:8px;"><img src="'.JURI::base().'components/com_jdownloads/assets/images/info32.png" align="middle" width="32" height="32" border="0" alt="info" /> '.$regg.'</div>', $body);                    
            } else {
                $body = str_replace('{checkbox_top}', '', $body);                    
            }    
        } 
